...
function initPuzzleQueue() {
    // 100個のピースから先頭80個に制限する
    const limitedSolution = puzzleSolution.slice(0, 80);
    pieceQueue = limitedSolution.map((p, idx) => ({ ...p, targetIdx: idx + 1 }));
}
...
function drawBackground() {
    if (targetImage.complete && targetImage.naturalHeight !== 0) {
        ...
        for (let i = 0; i <= 20; i++) {
            context.beginPath(); context.moveTo(i, 0); context.lineTo(i, 20 + Y_OFFSET); context.stroke();
        }
        for (let i = 0; i <= 20 + Y_OFFSET; i++) {
...
function draw() {
    ...
    context.fillRect(0, Y_OFFSET, 20, 20);

    // 余白エリアを塗る（クリーム色）
    context.fillStyle = '#FFFDD0';
    context.fillRect(0, 0, 20, Y_OFFSET);
...
function checkClear() {
    ...
    if (player.piecesPlaced >= 80 && isClear) {
        gameClear();
    } else if (player.piecesPlaced >= 80 && !isClear) {
...
function updateScore() {
    ...
    document.getElementById('lines').innerText = 80 - player.piecesPlaced;
    ...
    if (linesD) linesD.innerText = 80 - player.piecesPlaced;
